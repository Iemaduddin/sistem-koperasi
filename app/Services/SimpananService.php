...
class SimpananService
{
        ...
        return [
            ...
            'simpanan' => Simpanan::with([
                ...
                'batch' => function ($q) {
                    ...
                    ])->select('id', 'kode_transaksi', 'anggota_id', 'tanggal_transaksi', 'user_id', 'total');
                },
            ])
                ->latest('created_at')
                ->paginate(25),
    ...
    /**
     * @return Collection<int, Simpanan>
     */
    public function getTransactionsByAnggota(string $anggotaId): Collection
    {
        return Simpanan::with([
            'rekeningSimpanan' => function ($q) {
                $q->with([
                    'anggota' => fn($q2) => $q2->select('id', 'no_anggota', 'nama'),
                    'jenisSimpanan' => fn($q2) => $q2->select('id', 'kode', 'nama', 'jumlah_maksimum'),
            ...
            ->whereHas('rekeningSimpanan', function ($q) use ($anggotaId) {
    ...
    public function tarikOperasional(array $data): Simpanan
    {
        return DB::transaction(function () use ($data): Simpanan {
            $timestamp = Carbon::parse((string) ($data['created_at'] ?? now()));
            $jumlah = $this->toAmount($data['jumlah']);
            ...
            $operasionalJenis = $this->getJenisSimpananByKode()['OPERASIONAL'];
            $rekeningOperasional = RekeningSimpanan::query()
                ->where('anggota_id', $data['anggota_id'])
                ->where('jenis_simpanan_id', $operasionalJenis->id)
                ->lockForUpdate()
                ->first();

            if ($rekeningOperasional === null) {
                throw new RuntimeException('Anggota belum memiliki rekening simpanan operasional.');
            }

            $saldoOperasionalSaatIni = (float) $rekeningOperasional->saldo;
            if ($saldoOperasionalSaatIni < $jumlah) {
                throw new RuntimeException('Saldo simpanan operasional anggota tidak mencukupi untuk ditarik.');
            }

            $keterangan = $this->normalizeText($data['keterangan'] ?? null);

            $rekeningOperasional->saldo = round($saldoOperasionalSaatIni - $jumlah, 2);
    ...
    private function getJenisSimpananByKode(): array
    {
        $jenis = JenisSimpanan::query()
            ->whereIn(DB::raw('UPPER(kode)'), ['POKOK', 'WAJIB', 'SUKARELA', 'OPERASIONAL'])
            ->get();
        ...
        foreach (['POKOK', 'WAJIB', 'SUKARELA', 'OPERASIONAL'] as $kode) {
